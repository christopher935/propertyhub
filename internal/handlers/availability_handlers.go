...
// Deprecated: Use Gin routes instead via RegisterAvailabilityRoutesGin
...
// CheckAvailabilityGin is Gin-compatible version - supports property_id lookup
func (h *AvailabilityHandler) CheckAvailabilityGin(c *gin.Context) {
	...
	if propertyID != "" && mlsID == "" {
		var property struct {
			MLSId string `gorm:"column:mls_id"`
		}
...
// GetBlackoutDatesGin is Gin-compatible version
...
// CreateBlackoutDateGin is Gin-compatible version
...
// RemoveBlackoutDateGin is Gin-compatible version
func (h *AvailabilityHandler) RemoveBlackoutDateGin(c *gin.Context) {
	...
	if err := h.availabilityService.RemoveBlackoutDate(uint(id)); err != nil {
		if strings.Contains(err.Error(), "not found") {
			c.JSON(http.StatusNotFound, gin.H{"success": false, "error": "Blackout date not found"})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
...
// GetUpcomingBlackoutsGin is Gin-compatible version
...
// GetAvailabilityStatsGin is Gin-compatible version
...
// ValidateBookingGin is Gin-compatible version
...
// CleanupExpiredBlackoutsGin is Gin-compatible version
