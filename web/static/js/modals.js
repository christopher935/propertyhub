    ...
    prompt(message, defaultValue = '', title = 'Input') {
        return new Promise(resolve => {
            ...
            container.querySelector('.modal-body').innerHTML = `
                <p>${message}</p>
                <input type="text" id="modal-input" class="form-input" value="${defaultValue}">
            `;
            container.querySelector('.modal-footer').innerHTML = `
                <button class="btn btn-secondary" onclick="modalManager.close(); modalManager._resolve(null)">Cancel</button>
                <button class="btn btn-primary" onclick="modalManager._submitPrompt()">OK</button>
            `;
            container.classList.add('active');
            this._resolve = resolve;
...
window.showPrompt = (msg, def, title) => modalManager.prompt(msg, def, title);
